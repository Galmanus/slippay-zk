# Slippay Confidential Compliance — design
...
The singular concept: one succinct ZK proof that a BATCH of the Slippay agent's payments
...
company's money?" — and it builds on Slippay's existing "spend limit proven on-chain" copy.
...
- **Verifier** (Soroban, Rust): fork `stellar/soroban-examples/groth16_verifier` (BLS12-381, ark-bls12-381). On success, stamps the linked Slippay charge as compliant.
...
| 06-23→24 | wire to Slippay charge path (stamp charge compliant) | reuses existing rail, low |
