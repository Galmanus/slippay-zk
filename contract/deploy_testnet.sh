#!/usr/bin/env bash
# Deploy the Slippay ZK verifier to Stellar TESTNET and verify the mandate_sd proof.
# Free (friendbot-funded). Reproduces the live testnet artifact.
set -euo pipefail
cd "$(dirname "$0")"

IDENT="${IDENT:-slippayzk}"
...
cmd = ["stellar","contract","invoke","--id",cid,"--source","slippayzk",
