#!/usr/bin/env bash
# Deploy the Slippay ZK verifier to Stellar MAINNET and verify the mandate_sd proof.
#
...
#   3. Explicit operator intent: set SLIPPAY_MAINNET_GO=1 to arm it.
#
...
#
# Usage:  MAINNET_IDENT=<your-funded-key> SLIPPAY_MAINNET_GO=1 ./deploy_mainnet.sh
set -euo pipefail
cd "$(dirname "$0")"

: "${MAINNET_IDENT:?set MAINNET_IDENT to a funded mainnet stellar-cli identity}"
if [ "${SLIPPAY_MAINNET_GO:-0}" != "1" ]; then
  echo "Refusing to deploy to mainnet without SLIPPAY_MAINNET_GO=1 (real money)."; exit 1
fi
...
CID=$(stellar contract deploy \
  --wasm target/wasm32v1-none/release/slippay_zk_verifier.wasm \
