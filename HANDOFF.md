# Slippay ZK — handoff (2026-06-14)

Built autonomously while you were away. Everything below is **measured, not promised.**

## TL;DR

A ZK adaptation of Slippay for the Stellar Hacks: Real-World ZK hackathon (opens 06-15, closes 06-29).
Core concept — **provable bounded autonomy + selective disclosure**: one Groth16 proof that the Slippay
...
cd ref-bn254 && cargo test -p groth16-verifier slippay_sd_onchain -- --nocapture
...
1. **Wire to the real Slippay charge path.** The agent's actual payments feed the circuit's private inputs. Currently demo values.
...
4. **Mainnet (optional flex).** `contract/deploy_mainnet.sh` is ready but NOT run. It needs a funded mainnet key + `SLIPPAY_MAINNET_GO=1`. Real money, unaudited — your call, your keys.
